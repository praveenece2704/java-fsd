...
function existingdetails(){
    if (localStorage.getItem("myData") ===null){
        //alert("inside if")
        myData = [new Main(0,"Microsoft","Bug Fix - known issue","RajKrishna",100000)]
...
function CreateTableFromJSON() {    
    ...
    <td><button class="btn btn-primary" onclick="DeleteRow(${value.dealId})"> Delete </button></td>
  </tr>`);
    })
}
...
function InsertRow(dealId, clientName, projectName, projectManager, projectCost) {
    var a= new Main(dealId, clientName, projectName, projectManager, projectCost);
    myData.push(a);
    currentDealId++;
    localStorage.clear();
    localStorage.setItem("myData", JSON.stringify(myData)) 
